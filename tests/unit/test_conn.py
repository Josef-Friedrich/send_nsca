...
class TestConnectionLogic(TestCase):
    host_one = '10.0.0.1'
    addrinfo_one = (socket.AF_INET, socket.SOCK_STREAM,
                    socket.SOL_TCP, '', ('10.0.0.1', DEFAULT_PORT))
    host_two = '10.0.0.2'
    addrinfo_two = (socket.AF_INET, socket.SOCK_STREAM,
                    socket.SOL_TCP, '', ('10.0.0.2', DEFAULT_PORT))
    host_three = '10.0.0.3'
    addrinfo_three = (socket.AF_INET, socket.SOCK_STREAM,
                      socket.SOL_TCP, '', ('10.0.0.3', DEFAULT_PORT))
    host_four = '10.0.0.4'
    addrinfo_four = (socket.AF_INET, socket.SOCK_STREAM,
                     socket.SOL_UDP, '', ('10.0.0.4', DEFAULT_PORT))
    sigil_one = object()
    sigil_two = object()
    ...
    def test_sock_connect_timeout(self):
        ...
        with mock.patch('socket.getaddrinfo', mock_getaddrinfo):
            with mock.patch('socket.socket', mock_socket):
                self.sender._sock_connect(
                    'foo', 1, timeout=test_timeout, connect_all=False)
        self.assertEqual(mock_socket.call_count, 1)
        mock_socket.return_value.settimeout.assert_called_once_with(
            test_timeout)
        mock_socket.return_value.connect.assert_called_once_with(
            (self.host_one, DEFAULT_PORT))
    ...
    def test_sock_connect_many(self):
        ...
        mock_socket.return_value.connect.assert_any_call(
            (self.host_one, DEFAULT_PORT))
        mock_socket.return_value.connect.assert_any_call(
            (self.host_two, DEFAULT_PORT))
        mock_socket.return_value.connect.assert_any_call(
            (self.host_three, DEFAULT_PORT))
